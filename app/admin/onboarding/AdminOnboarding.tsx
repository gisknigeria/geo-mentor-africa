...
type Application = { id: string; application_type: string; organization_name: string | null; country_code: string; state_region: string; city: string; credentials_summary: string | null; motivation: string; status: string; created_at: string };
...
export function AdminOnboarding() {
  ...
  async function reviewApplication(id: string, decision: "VERIFIED" | "REJECTED" | "NEEDS_CHANGES") {
    ...
    setMessage(error ? "This action requires a verified platform administrator and the onboarding migrations." : `Application marked ${decision.toLowerCase().replace("_", " ")}.`);
  ...
  <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><Tabs defaultValue="applications"><TabsList className="gap-6 overflow-x-auto"><TabsTrigger value="applications">Applications ({applications.length})</TabsTrigger><TabsTrigger value="students">Student consent ({students.length})</TabsTrigger><TabsTrigger value="codes">Class codes</TabsTrigger><TabsTrigger value="staff">Staff invitations</TabsTrigger></TabsList>
