    ...
    <div className="workspace-frame">
      ...
      <section className="mt-6 grid gap-5 xl:grid-cols-2">
        <article className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div className="relative mt-4 h-64 overflow-hidden rounded-xl border border-slate-200 bg-[#e7efe1]"><SchoolLocationMap observations={mapObservations.flatMap((item) => item.latitude !== null && item.longitude !== null ? [{ id: item.id, latitude: item.latitude, longitude: item.longitude, label: item.common_name || item.observation_type, details: `${item.observation_type} · ${stageLabel[item.review_stage] || item.verification_status} · ${new Date(item.observed_at).toLocaleDateString()}` }] : [])} schoolLocation={schoolCoordinates} schoolName={dashboard.school.name} boundary={schoolBoundary} boundaryMode={boundaryMode} boundaryPoints={boundaryMode ? boundaryDraft : undefined} onChoose={(point) => void saveSchoolLocation(point.latitude, point.longitude)} onBoundaryPoint={(point) => setBoundaryDraft((current) => [...current, point])} onBoundaryChange={setBoundaryDraft} /></div>
          ...
          <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => { setBoundaryMode(true); setBoundaryDraft(schoolBoundary); setLocationMessage(schoolBoundary.length >= 3 ? "Drag boundary points to correct or add points around the perimeter." : "Click around the school perimeter to add boundary points."); }} className="min-h-10 rounded-lg border border-orange-300 bg-orange-50 px-4 text-xs font-bold text-orange-800">{schoolBoundary.length >= 3 ? "Edit school boundary" : "Map school boundary"}</button>{boundaryMode && <button type="button" onClick={() => void saveBoundary()} disabled={boundaryDraft.length < 3} className="min-h-10 rounded-lg bg-orange-600 px-4 text-xs font-bold text-white disabled:opacity-50">Save boundary ({boundaryDraft.length} points)</button>}{boundaryMode && <button type="button" onClick={() => { setBoundaryMode(false); setBoundaryDraft([]); }} className="min-h-10 rounded-lg border border-slate-300 px-4 text-xs font-bold text-slate-600">Cancel</button>}</div>
