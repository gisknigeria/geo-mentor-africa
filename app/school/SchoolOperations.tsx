...
const previewDashboard: Dashboard = {
  school: { id: "preview", name: "Staff School, Ibadan", country_code: "NG" }, role: "SCHOOL_ADMIN",
  metrics: { verified_students: 47, pending_students: 6, teacher_review: 7, expert_review: 3, verified_observations: 38, active_projects: 2 },
  recent_observations: [
    { id: "GMA-0248", observation_type: "TREE", common_name: "African tulip tree", scientific_name: "Spathodea campanulata", verification_status: "VERIFIED", review_stage: "CLOSED", observed_at: "2026-08-24T10:42:00Z", sensitivity_level: "STANDARD", latitude: 7.4408, longitude: 3.9002 },
    { id: "GMA-0246", observation_type: "POLLINATOR", common_name: "Plain tiger", scientific_name: null, verification_status: "PENDING", review_stage: "TEACHER_REVIEW", observed_at: "2026-08-23T15:18:00Z", sensitivity_level: "STANDARD", latitude: 7.4412, longitude: 3.8998 },
    { id: "GMA-0241", observation_type: "TREE", common_name: "Neem", scientific_name: null, verification_status: "PENDING", review_stage: "EXPERT_REVIEW", observed_at: "2026-08-23T11:06:00Z", sensitivity_level: "STANDARD", latitude: 7.4404, longitude: 3.9007 },
  ],
};
...
export function SchoolOperations() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    if (!error && data) { setDashboard(data as Dashboard); setPreview(false); }
  }, []);
  ...
  return <main className="min-h-screen bg-[#f4f6f1] text-[#15342d]">
    ...
    <div className="mx-auto max-w-7xl px-4 py-7 sm:px-7">
      ...
      {preview && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-900">Preview data is shown. Live school metrics activate after migration 0006 and verified school staff sign-in.</div>}
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,.7fr)]">
        ...
        <aside className="grid gap-5"><article className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="relative h-52 bg-[radial-gradient(circle_at_22%_35%,#b8d47b_0_5px,transparent_6px),radial-gradient(circle_at_70%_62%,#5f9770_0_6px,transparent_7px),linear-gradient(135deg,#e8eddc,#c6d7bd)]"><span className="absolute left-[42%] top-[42%] grid size-10 place-items-center rounded-full border-4 border-white bg-emerald-700 text-white shadow-lg"><MapPin className="size-4" /></span><span className="absolute bottom-3 left-3 rounded-md bg-white/90 px-2 py-1 text-[9px] font-bold text-emerald-900">Sensitive records hidden</span></div><div className="p-5"><p className="text-[9px] font-black tracking-[.15em] text-slate-400">PRIVACY-SAFE MAP</p><h2 className="mt-1 font-serif text-xl">School biodiversity map</h2><p className="mt-2 text-xs leading-5 text-slate-500">Only approximate coordinates are returned here. Critical species locations are withheld.</p></div></article><article className="rounded-2xl bg-[#0b4436] p-5 text-white"><p className="text-[9px] font-black tracking-[.15em] text-emerald-100/70">QUICK ACTIONS</p><div className="mt-4 grid gap-2"><Link href="/admin/onboarding" className="flex items-center justify-between rounded-lg bg-white/10 px-4 py-3 text-xs font-bold">Review students <UserCheck className="size-4" /></Link><Link href="/admin/onboarding" className="flex items-center justify-between rounded-lg bg-white/10 px-4 py-3 text-xs font-bold">Manage class codes <Users className="size-4" /></Link><Link href="/teacher" className="flex items-center justify-between rounded-lg bg-lime-300 px-4 py-3 text-xs font-bold text-emerald-950">Review evidence <ClipboardCheck className="size-4" /></Link></div><div className="mt-4 flex items-center gap-2 border-t border-white/10 pt-4 text-[10px] text-emerald-100/65"><FolderKanban className="size-4" />{dashboard.metrics.active_projects} active school projects</div></article></aside>
      </section>
    </div>
  </main>;
}
