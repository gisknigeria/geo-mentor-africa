...
export function AuthForm() {
  ...
  async function sendPasswordSetup() {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email: email.trim(),
      options: { shouldCreateUser: false, emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent("/auth/set-password?next=/portal")}` },
    });
    setMessage(error ? "We could not send the password setup email. Check the email address and try again." : "Check your email for a secure link to set or reset your password.");
    setBusy(false);
  }
  ...
  return (
    <main className="min-h-screen bg-[#f4f6f1] px-4 py-8 text-[#15342d] sm:grid sm:place-items-center">
      <section className="mx-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_18px_60px_rgba(17,59,47,.1)] sm:p-8">
        ...
        <button type="button" onClick={() => void sendPasswordSetup()} disabled={busy} className="mt-4 w-full text-xs font-bold text-emerald-800 hover:underline">Set or reset my password by email</button>
