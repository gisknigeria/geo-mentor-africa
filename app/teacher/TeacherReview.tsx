...
export function TeacherReview() {
  ...
  useEffect(() => {
    ...
    void Promise.resolve(
      ...
      if (active) setEvidenceUrl(result?.data?.signedUrl ?? null);
    });
    return () => {
      active = false;
    };
  ...
  async function submitReview(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    setMessage(null);
    const { error } = await supabase.rpc("review_observation_as_teacher", {
      target_observation: record.id,
      teacher_decision: decision,
      review_notes: notes.trim(),
    });
    setMessage(
      error
        ? "The review could not be saved. Confirm your teacher role and the review-workflow migration."
        : decision === "SUBMIT_TO_EXPERT"
          ? "Observation submitted to the expert validation queue."
          : "Student observation status updated.",
    );
    if (!error) {
      setNotes("");
      await loadQueue();
  ...
  const moveObservation = useCallback(
    (id: string, latitude: number, longitude: number) => {
      ...
      setLocationMessage("Location moved. Save it when the marker is correct.");
    },
    [],
  );

  async function saveObservationLocation() {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f6f1] px-4">
        <section className="max-w-md rounded-2xl bg-white p-8 text-center shadow-sm">
          ...
          <h1 className="mt-4 font-serif text-3xl text-emerald-950">
            Teacher sign-in required
          </h1>
          <p className="mt-3 text-sm text-slate-600">
            Only verified school staff can review student evidence.
          </p>
          <Link
            href="/auth"
    ...
    <main className="min-h-screen bg-[#f4f6f1] text-[#15342d]">
      <header className="border-b border-slate-200 bg-white px-4 py-4 sm:px-7">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <Logo />
          <span className="rounded-full bg-emerald-50 px-3 py-2 text-[10px] font-black text-emerald-800">
            TEACHER REVIEW
          </span>
        </div>
      </header>
      <div className="mx-auto grid max-w-7xl gap-5 px-4 py-7 lg:grid-cols-[360px_minmax(0,1fr)] lg:px-7">
        <aside className="rounded-2xl border border-slate-200 bg-white p-5">
          <Link href="/" className="text-xs font-bold text-emerald-800">
            ← Back to overview
          </Link>
          <p className="mt-7 text-[10px] font-bold tracking-[.18em] text-emerald-700">
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500">
            Check safety, evidence and learning quality before an expert sees
            the record.
          </p>
          {preview && (
            <div className="mt-4 rounded-lg bg-amber-50 p-3 text-[10px] leading-4 text-amber-900">
              Preview records are shown until a verified teacher signs in and
              migration 0005 is active.
            </div>
          )}
            ...
            {records.length === 0 ? (
              ...
              records.map((item, index) => (
                <button
                  ...
                  className={`grid grid-cols-[40px_1fr_auto] items-center gap-3 rounded-xl border p-3 text-left ${selected === index ? "border-emerald-700 bg-emerald-50" : "border-slate-200 hover:border-emerald-300"}`}
                >
                  <span className="grid size-10 place-items-center rounded-lg bg-lime-100 text-emerald-800">
                    <Camera className="size-4" />
                  </span>
                  <span>
                    <strong className="block text-xs text-emerald-950">
                      {item.common_name || item.observation_type}
                    </strong>
          ...
          {record ? (
            <div className="grid gap-5">
              <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div className="grid min-h-64 place-items-center bg-[radial-gradient(circle_at_30%_30%,rgba(199,226,107,.35),transparent_28%),linear-gradient(145deg,#dfead2,#b5cba8)]">
                  {evidenceUrl ? (
                    <img
                      src={evidenceUrl}
                      alt="Student biodiversity evidence"
                ...
                <div className="grid gap-4 border-t border-slate-200 p-5 sm:grid-cols-4">
                  <span>
                    <small className="block text-[9px] font-black tracking-wider text-slate-400">
                      SCHOOL
                    </small>
                    <strong className="mt-1 block text-xs">
                      {record.school?.name || "Assigned school"}
                    </strong>
                  </span>
                  <span>
                    <small className="block text-[9px] font-black tracking-wider text-slate-400">
                  ...
                  <span>
                    ...
                    <strong className="mt-1 flex items-center gap-1 text-xs">
                      <MapPin className="size-3" />±
                      {Math.round(record.coordinate_accuracy_m || 0)} m
                    </strong>
                  </span>
                  <span>
                    <small className="block text-[9px] font-black tracking-wider text-slate-400">
              ...
              <article className="relative h-72 overflow-hidden rounded-2xl border border-slate-200 bg-[#e7efe1]">
                ...
                />
                <div className="pointer-events-none absolute bottom-3 left-3 z-10 rounded-lg bg-white/90 px-3 py-2 text-[10px] font-bold text-slate-600">Drag the capture marker to correct its location</div>
              </article>
              <button type="button" onClick={() => void saveObservationLocation()} disabled={!locationChanged} className="min-h-11 rounded-lg bg-orange-600 px-4 text-xs font-black text-white disabled:opacity-50">Save location</button>
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold tracking-[.18em] text-emerald-700">
                      TEACHER DECISION
                    </p>
                ...
                <blockquote className="mt-5 rounded-r-lg border-l-4 border-lime-500 bg-lime-50 p-4 text-sm leading-6 text-slate-700">
                  {record.notes}
                </blockquote>
                  ...
                  <div className="mt-2 grid gap-3 sm:grid-cols-3">
                    {[
                      [
                        "SUBMIT_TO_EXPERT",
                        "Send to expert",
                        "Evidence is safe and useful",
                      ],
                      [
                        "NEEDS_CHANGES",
                        "Needs changes",
                        "Student should improve it",
                      ],
                      ["REJECTED", "Reject", "Unsafe or unusable record"],
                    ].map(([value, label, help]) => (
                      <label
                        key={value}
                        className={`cursor-pointer rounded-xl border p-4 ${decision === value ? "border-emerald-700 bg-emerald-50" : "border-slate-200"}`}
                      >
                        <input
                          className="sr-only"
                          type="radio"
                ...
                <label className="mt-5 grid gap-2 text-xs font-bold text-slate-700">
                  <span>Feedback for the student and expert</span>
                  <textarea
                    required
                    minLength={3}
                    maxLength={1000}
                    ...
                    className="min-h-28 rounded-lg border border-slate-300 p-3 text-sm font-normal"
                    placeholder="Explain the evidence quality and any changes needed…"
                  />
                </label>
                {message && (
                  <div
                    className="mt-4 rounded-lg bg-emerald-50 p-3 text-xs leading-5 text-emerald-900"
                    role="status"
                  >
                    {message}
                  </div>
                ...
                <div className="mt-5 flex justify-end">
                  <Button type="submit" size="lg" disabled={busy}>
                    ...
                    {busy ? "Saving review…" : "Save teacher decision"}
                  </Button>
                </div>
              </form>
            </div>
